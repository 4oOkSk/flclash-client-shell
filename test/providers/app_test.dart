...
void main() {
  ...
  group('Requests provider', () {
    test('replaces an active tracker with its closed snapshot', () {
      ...
      notifier.addRequest(active);
      notifier.addRequest(closed);

      final requests = container.read(requestsProvider).list;
      expect(requests, hasLength(1));
      expect(requests.single.lifecycle, 'closed');
      expect(requests.single.durationMs, 1200);

      notifier.addRequest(active.copyWith(upload: 999));

      final afterLateActive = container.read(requestsProvider).list;
      expect(afterLateActive, hasLength(1));
      expect(afterLateActive.single.lifecycle, 'closed');
      expect(afterLateActive.single.upload, 0);
    });
  });
