...
void main() {
  ...
  group('AppSettingProps JSON round-trip', () {
    test('default values survive round-trip', () {
      ...
      expect(
        ...
        isNot(contains(DashboardWidget.vpnButton)),
      );
    });
  ...
  group('NetworkProps JSON round-trip', () {
    ...
    test('round-trip with custom values', () {
      ...
      expect(props.toJson()['managedRouteMode'], 'bypass-overseas');
      ...
      expect(restored.managedRouteMode, ManagedRouteMode.bypassOverseas);
    });

    test(
      'migrates every legacy bypass flag combination without changing it',
      () {
        const cases = <(bool, bool, ManagedRouteMode)>[
        ...
        for (final (bypassMainland, bypassOverseas, expected) in cases) {
          final restored = NetworkProps.fromJson({
            'bypassMainland': bypassMainland,
            'bypassOverseas': bypassOverseas,
          });
          expect(restored.managedRouteMode, expected);
        }
      },
    );

    test('new managed mode wins over rollback compatibility flags', () {
      final restored = NetworkProps.fromJson({
        'managedRouteMode': 'bypass-overseas',
        'bypassMainland': true,
        'bypassOverseas': true,
      });
      expect(restored.managedRouteMode, ManagedRouteMode.bypassOverseas);
    ...
    test('rejects an unknown new mode instead of reading legacy flags', () {
      expect(
        () => NetworkProps.fromJson({
          'managedRouteMode': 'future-mode',
          'bypassMainland': true,
          'bypassOverseas': false,
        }),
        throwsFormatException,
      );
      expect(
        () => NetworkProps.fromJson({'managedRouteMode': null}),
        throwsFormatException,
      );
    });

    test('legacy route flags keep null defaults and reject wrong types', () {
      expect(
        NetworkProps.fromJson({
          'bypassMainland': null,
          'bypassOverseas': null,
      ...
      for (final malformed in <Map<String, Object?>>[
        ...
        expect(() => NetworkProps.fromJson(malformed), throwsFormatException);
      }
    });

    test('compatible JSON writes the mode and matching legacy flags', () {
      const props = NetworkProps(
        managedRouteMode: ManagedRouteMode.directAllLegacy,
      );
      final json = networkPropsToCompatibleJson(props);

      expect(json['managedRouteMode'], 'direct-all');
      expect(json['bypassMainland'], isTrue);
      expect(json['bypassOverseas'], isTrue);
    });

    test('compatible config JSON dual-writes nested network props', () {
      const config = Config(
      ...
      final json = configToCompatibleJson(config);
      final network = json['networkProps'] as Map<String, dynamic>;

      expect(network['managedRouteMode'], 'bypass-overseas');
      expect(network['bypassMainland'], isFalse);
      expect(network['bypassOverseas'], isTrue);
    });
  });
