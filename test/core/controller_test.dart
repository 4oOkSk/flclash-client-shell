...
void main() {
  ...
  group('lifecycle methods', () {
    ...
    test('platform diagnostic logs delegate to interface', () async {
      when(
        () => mock.getPlatformDiagnosticLogs(),
      ).thenAnswer((_) async => ['native-log']);

      expect(await controller.getPlatformDiagnosticLogs(), ['native-log']);
      verify(() => mock.getPlatformDiagnosticLogs()).called(1);
    });
  ...
  group('config methods', () {
    ...
    test('setupFromClient forwards the local route overlay', () async {
      const overlay = PrivateRouteOverlay(rules: ['DOMAIN,example.com,DIRECT']);
      when(
        () => mock.setupFromClient(
          endpoint: 'https://example.com/client',
          refreshInterval: const Duration(days: 1),
          selectedMap: const {'Proxy': 'Tokyo'},
          testUrl: 'https://example.com/generate_204',
          routeOverlay: overlay,
        ),
      ).thenAnswer((_) async => 'ok');

      final result = await controller.setupFromClient(
        endpoint: 'https://example.com/client',
        refreshInterval: const Duration(days: 1),
        selectedMap: const {'Proxy': 'Tokyo'},
        testUrl: 'https://example.com/generate_204',
    ...
    test('clientClear forwards the endpoint', () async {
      when(
        () => mock.clientClear(endpoint: 'https://example.com/client'),
      ).thenAnswer((_) async => '');

      expect(
        await controller.clientClear(endpoint: 'https://example.com/client'),
        isEmpty,
      );
      verify(
        () => mock.clientClear(endpoint: 'https://example.com/client'),
