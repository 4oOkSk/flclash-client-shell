...
procedure StopLegacyHelperService;
var
  ResultCode: Integer;
begin
  Exec(ExpandConstant('{sys}\sc.exe'), 'stop ' + LegacyHelperService, '',
...
var
  ...
  ResultCode: Integer;
begin
  Processes := ['HarborProxy.exe', 'HarborProxyCore.exe', 'HarborProxyHelperService.exe'];
...
begin
  ...
  if not Result then
    MsgBox('无法删除旧版 HarborProxy Helper 服务，请重启 Windows 后重试。',
      mbError, MB_OK);
end;
