...
class RuleItem extends StatelessWidget {
  ...
  VM2<bool, Color?> _checkInvalid(BuildContext context) {
    if (checkInvalidHandler?.call(rule) == true) {
      return VM2(true, context.colorScheme.error);
    }
...
class AddOrEditRuleDialog extends StatefulWidget {
  final Rule? rule;
  final List<String> ruleTargets;
  final List<String> ruleProviderNames;
  final bool allowRuleSet;

  const AddOrEditRuleDialog({
    super.key,
    this.rule,
    this.ruleTargets = const [],
    this.ruleProviderNames = const [],
...
class _AddOrEditRuleDialogState extends State<AddOrEditRuleDialog> {
  ...
  List<RuleAction> get _ruleActionOptions => [
    ...RuleAction.addedRuleActions,
    if (widget.allowRuleSet) RuleAction.RULE_SET,
  ];
  ...
  void _initState() {
    final targets = <String>{
      ...
      ...widget.ruleTargets.where((item) => item.trim().isNotEmpty),
      'MATCH',
    };
    _targetItems = targets
        .map((item) => DropdownMenuEntry(value: item, label: item))
        .toList();
    final rule = widget.rule;
    if (rule != null) {
      _ruleAction = rule.ruleAction;
      _contentController.text = rule.realContent ?? '';
  ...
  void _handleSubmit() {
    ...
    final rule = Rule(
      ...
      content: _ruleAction == RuleAction.RULE_SET
          ? null
          : _contentController.text,
      ruleProvider: _ruleAction == RuleAction.RULE_SET
          ? _contentController.text
          : null,
              ...
              return Column(
                ...
                children: [
                  FilledButton.tonal(
                    onPressed: () async {
                      ...
                      if (_ruleAction == RuleAction.RULE_SET &&
                          !widget.ruleProviderNames.contains(
                            _contentController.text,
                          )) {
                        _contentController.text =
                            widget.ruleProviderNames.firstOrNull ?? '';
                      }
                      setState(() {});
                    },
                  ...
                  if (_ruleAction == RuleAction.RULE_SET)
                    FormField<String>(
                      validator: (_) {
                        if (!widget.ruleProviderNames.contains(
                          _contentController.text,
                        )) {
                      ...
                      builder: (field) {
                        return DropdownMenu<String>(
                          ...
                          label: Text(appLocalizations.ruleSet),
                          width: 280,
                          menuHeight: 250,
                          enableFilter: false,
                          enableSearch: false,
                          dropdownMenuEntries: widget.ruleProviderNames
                              .map(
                                (item) =>
