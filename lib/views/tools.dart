...
class _ToolViewState extends ConsumerState<ToolsView> {
  Widget _buildNavigationMenuItem(NavigationItem navigationItem) {
    return ListItem.open(
      ...
      title: Text(navigationItemLabel(context, navigationItem)),
  ...
  List<Widget> _getOtherList(bool enableDeveloperMode) {
    return generateSection(
      ...
      items: [
        if (kClientApiBase.isNotEmpty) const _ClientClearItem(),
...
class _ClientClearItem extends ConsumerWidget {
  const _ClientClearItem();

  Future<void> _handleClear(BuildContext context) async {
    final res = await globalState.showMessage(
      title: context.appLocalizations.clearData,
      message: TextSpan(text: context.appLocalizations.confirmClearAllData),
      confirmText: context.appLocalizations.confirm,
    );
    if (res != true) return;
    final message = await coreController.clientClear(endpoint: kClientApiBase);
    ...
    try {
      ...
      // TUN alive.  Make one direct stop attempt before deleting the key and
      // exiting even if the provider-side state transition failed.
      try {
        await coreController.stopListener();
      } catch (_) {}
    }
    await clientSecureStorage.delete();
    await preferences.clearPrivateClientSelectedMap();
    await globalState.container
