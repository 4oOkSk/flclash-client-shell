...
class GlobalState {
  ...
  Future<ProviderContainer> _initData(int version) async {
    ...
    final loadedConfig = await migration.migrationIfNeeded(
      configMap,
      sync: (data) async {
    ...
    final initializePrivateDashboardCards =
        kPrivateClientMode &&
        !await preferences.getPrivateDashboardCardsInitialized();
    final config = kPrivateClientMode
        ? normalizePrivateClientConfig(
            loadedConfig,
            initializeDashboardCards: initializePrivateDashboardCards,
          )
        : loadedConfig;
    var configSaved = true;
    if (config != loadedConfig) {
      configSaved = await preferences.saveConfig(config);
    }
    if (initializePrivateDashboardCards && configSaved) {
      await preferences.setPrivateDashboardCardsInitialized();
    }
  ...
  Future<void> _showCrashlyticsTip() async {
    if (kPrivateClientMode || !system.isAndroid) return;
    if (container.read(
