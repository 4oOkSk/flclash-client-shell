...
abstract class NetworkProps with _$NetworkProps {
  const factory NetworkProps({
    ...
    @Default(ManagedRouteMode.bypassMainland) ManagedRouteMode managedRouteMode,
  }) = _NetworkProps;

  factory NetworkProps.fromJson(Map<String, Object?>? json) => json == null
      ? const NetworkProps()
      : _$NetworkPropsFromJson(_normalizeNetworkPropsJson(json));
}

Map<String, Object?> _normalizeNetworkPropsJson(Map<String, Object?> json) {
  final normalized = Map<String, Object?>.from(json);
  if (json.containsKey('managedRouteMode')) {
    final currentMode = ManagedRouteMode.tryParse(json['managedRouteMode']);
    if (currentMode == null) {
      throw const FormatException('invalid managed route mode');
    }
    normalized['managedRouteMode'] = currentMode.wireValue;
    return normalized;
  }
  final legacyBypassMainland = json['bypassMainland'];
  final legacyBypassOverseas = json['bypassOverseas'];
  normalized['managedRouteMode'] = ManagedRouteMode.fromLegacyFlags(
    bypassMainland: _legacyManagedRouteFlag(
      legacyBypassMainland,
      defaultValue: true,
    ),
    bypassOverseas: _legacyManagedRouteFlag(
      legacyBypassOverseas,
      defaultValue: false,
    ),
  ).wireValue;
  return normalized;
...
bool _legacyManagedRouteFlag(Object? value, {required bool defaultValue}) {
  if (value == null) return defaultValue;
  if (value is bool) return value;
  throw const FormatException('invalid legacy managed route flags');
}

/// Writes the single effective mode plus the two legacy projections for one
/// rollback-compatible release. Runtime code must only read [managedRouteMode].
Map<String, dynamic> networkPropsToCompatibleJson(NetworkProps props) {
  return {
    ...props.toJson(),
    'managedRouteMode': props.managedRouteMode.wireValue,
    'bypassMainland': props.managedRouteMode.bypassesMainland,
    'bypassOverseas': props.managedRouteMode.bypassesOverseas,
  };
}
...
/// Serializes persisted and backed-up configuration with route compatibility.
Map<String, dynamic> configToCompatibleJson(Config config) {
  return {
    ...config.toJson(),
    'networkProps': networkPropsToCompatibleJson(config.networkProps),
  };
}
