...
final privateClientAccountInfoProvider =
    FutureProvider<PrivateClientAccountInfo?>((ref) async {
      if (!kPrivateClientMode) return null;
      final raw = await coreController.clientAccountInfo();
      return PrivateClientAccountInfo.tryParse(raw);
    });
    ...
    patchClashConfigProvider.select(
      (state) => UpdateParams(
        tun: state.tun
            ...
            .copyWith(enable: kPrivateClientMode ? true : state.tun.enable),
        allowLan: kPrivateClientMode ? false : state.allowLan,
        findProcessMode: state.findProcessMode,
        mode: kPrivateClientMode ? Mode.rule : state.mode,
        logLevel: state.logLevel,
        ipv6: effectiveClientCoreIpv6(
...
ProxyState proxyState(Ref ref) {
  ...
  return ProxyState(
    ...
    systemProxy: kPrivateClientMode ? false : vm2.a,
    bassDomain: vm2.b,
    port: mixedPort,
  );
}
...
TrayState trayState(Ref ref) {
  ...
  return TrayState(
    mode: kPrivateClientMode ? Mode.rule : clashConfigVm3.a,
    port: clashConfigVm3.b,
    autoLaunch: appSettingVm3.a,
    systemProxy: kPrivateClientMode ? false : systemProxy,
    tunEnable: kPrivateClientMode ? true : clashConfigVm3.c,
    isStart: isStart,
...
DashboardState dashboardState(Ref ref) {
  final configuredWidgets = ref.watch(
    appSettingProvider.select((state) => state.dashboardWidgets),
  );
  return DashboardState(
    dashboardWidgets: kPrivateClientMode
        ? privateClientDashboardWidgets(configuredWidgets)
        : genericDashboardWidgets(configuredWidgets),
  );
}
...
Map<String, String> selectedMap(Ref ref) {
  if (kPrivateClientMode) {
    return Map<String, String>.unmodifiable(privateClientSelectedMap);
  }
...
SharedState sharedState(Ref ref) {
  ...
  final clashConfigVM2 = ref.watch(
    patchClashConfigProvider.select(
      (state) => VM2(state.tun.stack, state.mixedPort),
    ),
  );
  ...
  final stack = effectiveClientVpnStackName(
    configured: clashConfigVM2.a,
    privateClientMode: kPrivateClientMode,
  ...
  final effectiveIpv6 = effectiveClientIpv6(
    configured: vpnSetting.ipv6,
    privateClientMode: kPrivateClientMode,
    isAndroid: system.isAndroid,
  );
  return SharedState(
    currentProfileName: kPrivateClientMode ? appName : currentProfileName,
    ...
    vpnOptions: VpnOptions(
      enable: kPrivateClientMode ? true : vpnSetting.enable,
      stack: stack,
      systemProxy: effectiveClientVpnSystemProxy(
        configured: vpnSetting.systemProxy,
        privateClientMode: kPrivateClientMode,
        isAndroid: system.isAndroid,
      ),
      port: port,
