...
class Requests extends _$Requests with AutoDisposeNotifierMixin {
  ...
  void addRequest(TrackerInfo value) {
    ...
    if (value.id.isEmpty) {
      ...
      final previousIndex = next.list.indexWhere((item) => item.id == value.id);
      if (previousIndex >= 0 &&
          next[previousIndex].lifecycle == 'closed' &&
          value.lifecycle != 'closed') {
        return;
      }
      next.addOrReplace(value, (item) => item.id == value.id);
    }
    this.value = next;
  }
}
...
class DelayDataSource extends _$DelayDataSource with AutoDisposeNotifierMixin {
  ...
  void setDelay(Delay delay) {
    if (state[delay.url]?[delay.name] != delay.value) {
      ...
      newDelayMap[delay.url] = {...?state[delay.url], delay.name: delay.value};
      value = newDelayMap;
    }
  }
}
