...
class CommonAction extends _$CommonAction {
  ...
  Future<void> autoCheckUpdate() async {
    // Private releases are delivered through the HarborProxy release channel,
    // not the public upstream FlClash feed.
    if (kPrivateClientMode) return;
...
class SetupAction extends _$SetupAction {
  ...
  Future<void> _handleStart() async {
    if (!ref.read(suspendProvider)) {
      ...
      if (!listenerStarted) {
        throw StateError('core listener failed to start');
      }
    }
    startTime ??= DateTime.now();
    // Only expose the running state after the core listener is ready.  On
    // desktop this prevents the OS proxy from pointing at a closed port.
  ...
  Future<void> initStatus() async {
    ...
    if (kClientApiBase.isNotEmpty) {
      final hasClientSession = await coreController.clientHasSession();
      if (!hasClientSession) {
        return;
      }
    }
  ...
  Future<void> updateStatus(bool isStart, {bool isInit = false}) async {
    if (isStart) {
      if (kPrivateClientMode) {
        ...
        if (networkProps.systemProxy) {
          ref
              .read(networkSettingProvider.notifier)
              .update((state) => state.copyWith(systemProxy: false));
        }
        final patchConfig = ref.read(patchClashConfigProvider);
        if (!patchConfig.tun.enable || patchConfig.mode != Mode.rule) {
          ref
      ...
      if (kPrivateClientMode && system.isAndroid) {
        ...
        final message =
            await service?.syncState(ref.read(sharedStateProvider)) ?? '';
      ...
      if (!isInit) {
        ...
        if (kPrivateClientMode && !this.isStart) {
          ref.read(runTimeProvider.notifier).value = null;
        }
      }
    } else {
  ...
  Future<String> _setupPrivateClientProfileImpl({
    ...
    final managedRouting = PrivateManagedRouting(
      mode: networkProps.managedRouteMode,
      // A managed client with core IPv6 disabled still captures IPv6 in TUN so
      // it cannot leak. Apps may learn literal destinations through HTTPDNS;
      // reject them quickly so the app retries the available IPv4 path. Keep
      // genuine desktop IPv6 when the user has enabled it.
      rejectIpv6:
          kPrivateClientMode &&
    ...
    final applyResult = await applyPrivateRouteOverlayFallbacks(
      ...
      apply: (overlay) {
        return coreController.setupFromClient(
    ...
    // The server owns the hidden proxy definitions, while ports, mode and TUN
    // are explicitly local user settings.  Applying the encrypted YAML alone
    // used to overwrite those local settings, leaving TUN disabled and making
    // the UI disagree with the running core.
    final updateParams = ref.read(updateParamsProvider);
    final adminResult = await _requestAdmin(updateParams.tun.enable);
    ...
    await ref.read(proxiesActionProvider.notifier).updateGroups();
    await ref.read(providersProvider.notifier).syncProviders();
    final fallbackNotification = resolvePrivateRouteFallbackNotification(
      fallback: routeOverlayFallback,
      wasNotified: _privateRouteFallbackNotified,
    );
    _privateRouteFallbackNotified = fallbackNotification.nextNotified;
    if (fallbackNotification.notify) {
  ...
  Future<Result<bool>> _requestAdmin(bool enableTun) async {
    final realTunEnable = ref.read(realTunEnableProvider);
    if (requiresPrivilegedTunCoreAuthorization(
      isAndroid: system.isAndroid,
      requestedTun: enableTun,
      effectiveTun: realTunEnable,
    )) {
      ...
      if (code == AuthorizeCode.success) {
        ...
        // The restarted private-client core received the encrypted base
        // config, but TUN is a local setting and still needs updateConfig.
        return Result.success(enableTun);
      }
      if (code != AuthorizeCode.none) {
        if (kPrivateClientMode) {
          return Result.error(
  ...
  Future<void> _setupConfig({
    ...
    // 入册模式: 配置来源为内嵌 enroll blob，从面板拉取后仅喂入内核内存。
...
class BackupAction extends _$BackupAction {
  ...
  Future<String> backup() async {
    ...
    final configMap = configToCompatibleJson(ref.read(configProvider));
...
class ProxiesAction extends _$ProxiesAction {
    ...
    try {
      ...
      ref.read(groupsProvider.notifier).value = await retry(
        task: () async {
          ...
          final selectedMap = kPrivateClientMode
              ? privateClientSelectedMap
              : ref.read(
  ...
  Future<void> changeProxy({
    ...
    if (kPrivateClientMode || ref.read(appSettingProvider).closeConnections) {
...
class ProfilesAction extends _$ProfilesAction {
  ...
  void updateCurrentSelectedMap(String groupName, String proxyName) {
    if (kPrivateClientMode) {
      final selectedMap = Map<String, String>.from(privateClientSelectedMap);
      ...
      privateClientSelectedMap
        ..clear()
        ..addAll(selectedMap);
      unawaited(preferences.savePrivateClientSelectedMap(selectedMap));
      return;
    }
    final currentProfile = ref.read(currentProfileProvider);
