...
func recordConnectionEnd(conn net.Conn, err error) {
	if recorder, ok := conn.(interface{ RecordEnd(error) }); ok {
		recorder.RecordEnd(err)
	}
}
...
func Relay(leftConn, rightConn net.Conn) {
	...
	go func() {
		...
		recordConnectionEnd(rightConn, err)
		if err == nil {
