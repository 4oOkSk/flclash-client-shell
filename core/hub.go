...
// PATCH(ops): 入册——解乱码 blob→拉配置→验签解密→内存喂入内核(不落盘、不显地址)
func handleSetupFromEnroll(data []byte) string {
	if !isInit {
		return "not initialized"
	}
	...
	case enrollModeClientLogin:
		...
		return ""
	case enrollModeClientState:
		if clientHasSession() {
			return ""
		}
		return "client login required"
	case enrollModeClientInfo:
	...
	case enrollModeClientSecureStore:
		...
		return ""
	case enrollModeClientClear:
		if err := clientClear(params.Endpoint); err != nil {
			return err.Error()
		}
		return ""
	}
	...
	if params.Mode == enrollModeClientSetup {
		cfg, err = fetchConfigFromClient(params.Endpoint, refreshInterval)
	} else {
		cfg, err = fetchConfigFromEnroll(params.Blob, refreshInterval)
	}
	if err != nil {
		if params.Mode == enrollModeClientSetup {
			clientRecordApply("config-fetch-failed")
		}
		return clientSetupErrorMessage(err)
	}
	effectiveConfig := cfg
	if params.Mode == enrollModeClientSetup && !params.RouteOverlay.empty() {
		effectiveConfig, err = applyClientRouteOverlay(cfg, params.RouteOverlay)
		if err != nil {
			clientRecordApply("route-overlay-failed")
			return clientRouteOverlayError(err)
		}
	}
	sp := defaultSetupParams()
	sp.Config = effectiveConfig
	sp.SelectedMap = params.SelectedMap
	if params.TestURL != "" {
		sp.TestURL = params.TestURL
	}
	...
	if params.Mode == enrollModeClientSetup {
		if err := commitPendingClientConfigCache(); err != nil {
			clientRecordApply("cache-commit-failed")
			return err.Error()
		}
...
func init() {
	...
	statistic.DefaultRequestNotify = func(c statistic.Tracker) {
		annotateClientTrackerDiagnostics(c.Info())
		sendMessage(Message{
			Type: RequestMessage,
			Data: c,
		})
	}
	statistic.DefaultRequestCloseNotify = func(c statistic.Tracker) {
		sendMessage(Message{
			Type: RequestMessage,
			Data: c.LifecycleInfo(),
		})
	}
	...
	updater.GeoUpdateHook = func(geoType string, updating bool, skipped bool, updateErr error) {
		...
		clientRecordGeoUpdate(geoType, result)
		sendMessage(Message{
