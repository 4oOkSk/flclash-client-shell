...
type SetupParams struct {
	...
	// PATCH(ops): 配置内容(YAML). 非空则内存喂入内核, 不读磁盘 config.yaml —— 隐藏地址核心
	Config string `json:"config"`
}

// PATCH(ops): 入册参数——不透明乱码 blob + 选中节点 + 测速URL
...
const (
	...
	setupFromEnrollMethod          Method = "setupFromEnroll" // PATCH(ops): 入册(解乱码→拉配置→内存喂入)
