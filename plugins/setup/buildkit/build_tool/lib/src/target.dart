...
class Target {
  ...
  static List<Target> resolveMacosTargets({String? archName}) {
    final requestedArch = archName ?? 'universal';
    if (requestedArch == 'universal') {
      return [macosArm64, macosAmd64];
    }
    final targets = forPlatform(
      'darwin',
    ).where((target) => target.goarch == requestedArch).toList();
    if (targets.isEmpty) {
      throw BuildException('Invalid macOS arch: $requestedArch');
    }
    return targets;
  }
